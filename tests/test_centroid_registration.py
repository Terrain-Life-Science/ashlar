...
def test_register_centroids_icp_simple():
    ...
    shift, error, metadata = register_centroids_icp(
        ref_centroids, target_centroids, distance_threshold=5.0, min_matches=2
    )
...
def test_register_centroids_icp_empty():
    ...
    shift, error, metadata = register_centroids_icp(ref_centroids, target_centroids)
...
def test_estimate_shift_from_centroids_synthetic():
    ...
    for cy, cx in centers:
        blob = np.exp(
            -((y - (cy + known_shift_y)) ** 2 + (x - (cx + known_shift_x)) ** 2) / (2 * 15**2)
        )
