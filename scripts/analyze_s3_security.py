...
class S3SecurityAnalyzer:
    ...
    def check_cloudfront_for_bucket(self, bucket_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'AccessDenied':
                result['recommendation'] = 'Access denied to CloudFront. May require additional permissions.'
            else:
        ...
        try:
            ...
            for statement in policy_doc.get('Statement', []):
                ...
                if principal == '*' or (isinstance(principal, dict) and principal.get('AWS') == '*'):
                    ...
                    result['allowed_actions'].extend(actions if isinstance(actions, list) else [actions])
    ...
    def analyze_bucket_contents(self, bucket_name: str, max_files: int = 100) -> Dict[str, Any]:
        ...
        sensitive_patterns = [
            '.key', '.pem', '.p12', '.pfx',  # Keys/certificates
            '.env', '.config', '.conf',      # Config files
            '.sql', '.db', '.sqlite',        # Databases
            'password', 'secret', 'credential', 'token',  # Keywords
            '.log', '.txt'  # May contain sensitive info
        ]
        ...
        try:
            ...
            if result['has_sensitive_patterns']:
                ...
                if 'html' in file_types or 'css' in file_types or 'js' in file_types:
                    result['analysis'] = 'Bucket appears to contain website files (HTML/CSS/JS). Public access may be intentional.'
                elif 'pdf' in file_types or 'doc' in file_types or 'docx' in file_types:
                    result['analysis'] = 'Bucket appears to contain documents. Verify if public access is needed.'
                elif 'jpg' in file_types or 'png' in file_types or 'gif' in file_types:
                    result['analysis'] = 'Bucket appears to contain images. Public access may be for media hosting.'
                else:
                    result['analysis'] = f'Bucket contains {result["total_files"]} files. Review contents to determine if public access is appropriate.'
            
        except ClientError as e:
            if e.response['Error']['Code'] == 'AccessDenied':
                result['analysis'] = 'Access denied to bucket contents. May require additional permissions.'
            else:
                result['analysis'] = f'Error analyzing bucket contents: {e}'
        
        return result
    
    def analyze_bucket_security(self, bucket_name: str, check_cloudfront: bool = True) -> Dict[str, Any]:
        """
    ...
    def analyze_all_public_buckets(self, check_cloudfront: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            results['summary'] = {
                ...
                'high_risk_buckets': len([b for b in results['public_buckets'] if b['risk_level'] == 'HIGH']),
                'medium_risk_buckets': len([b for b in results['public_buckets'] if b['risk_level'] == 'MEDIUM'])
            }
...
Examples:
  ...
  python scripts/analyze_s3_security.py --all --no-cloudfront
    ...
    parser.add_argument(
        '--bucket',
        type=str,
        help='Specific bucket name to analyze'
    )
    
    parser.add_argument(
        '--all',
        action='store_true',
        help='Analyze all buckets with public access'
    )
