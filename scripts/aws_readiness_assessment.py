...
class AWSReadinessAssessment:
        ...
        try:
            ...
            try:
                ...
                for key_metadata in access_keys.get('AccessKeyMetadata', []):
                    key_info = {
                        ...
                        'create_date': key_metadata.get('CreateDate').isoformat() if key_metadata.get('CreateDate') else None
                    }
            ...
            try:
                ...
                for policy in iam_assessment['policies'].get('managed_policies', []):
                    if 'AdministratorAccess' in policy.get('policy_arn', '') or 'PowerUserAccess' in policy.get('policy_arn', ''):
                        has_admin = True
                        self.results['risks'].append({
                            'severity': 'MEDIUM',
                            'category': 'Security',
    ...
    def assess_ec2(self) -> Dict[str, Any]:
        ...
        if self.results.get('account_info', {}).get('regions'):
            default_region = self.results['account_info']['regions'][0] if self.results['account_info']['regions'] else 'us-east-1'
        ...
        try:
            ...
            for reservation in instances.get('Reservations', []):
                for instance in reservation.get('Instances', []):
                    ...
                    if 'g4dn' in instance.get('InstanceType', '').lower() or 'g5' in instance.get('InstanceType', '').lower():
    ...
    def assess_s3(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for bucket in bucket_list:
                ...
                try:
                    ...
                    try:
                        ...
                        bucket_info['lifecycle_rules'] = [rule.get('Id') for rule in lifecycle.get('Rules', [])]
                    except ClientError as e:
            ...
            print(f"[3/3] Security review complete")
    ...
    def assess_networking(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print("\n[1/5] Checking VPCs...")
            vpcs = self.ec2.describe_vpcs()
            networking['vpcs'] = [{'vpc_id': vpc['VpcId'], 'cidr': vpc.get('CidrBlock')} 
                                 for vpc in vpcs.get('Vpcs', [])]
        ...
        try:
            ...
            try:
                ...
                if trail_list:
                    monitoring['cloudtrail'] = {
                        'trails': [{'name': t.get('Name'), 'is_logging': t.get('IsLogging')} 
                                 for t in trail_list],
                        'enabled': True
                    }
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'AccessDenied':
                    monitoring['config'] = {'status': 'Access denied - requires additional permissions'}
                else:
                    monitoring['config'] = {'error': str(e)}
                print(f"  AWS Config: Not accessible (may require additional permissions)")
    ...
    def assess_costs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.get('ResultsByTime'):
                ...
                cost_amount = float(cost_result.get('Total', {}).get('BlendedCost', {}).get('Amount', 0))
    ...
    def generate_markdown_report(self, output_file: Path):
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"**Account ID:** {self.results['account_info'].get('caller_identity', {}).get('account_id', 'N/A')}\n\n")
            ...
            f.write(f"- **Available Regions:** {len(self.results['account_info'].get('regions', []))}\n\n")
            ...
            buckets_with_encryption = sum(1 for b in s3.get('buckets', []) 
            ...
            if self.results['recommendations']:
                ...
                for rec in self.results['recommendations']:
                    f.write(f"- **{rec.get('category', 'General')}:** {rec.get('finding', 'N/A')}\n")
...
Examples:
  ...
  python scripts/aws_readiness_assessment.py --format markdown
    ...
    parser.add_argument(
        '--account-id',
        type=str,
        help='AWS Account ID to verify (default: auto-detect)'
    )
    ...
    # Run assessment
    assessment = AWSReadinessAssessment(
    ...
    try:
        ...
        print("=" * 60)
        print(f"Account ID: {results['account_info'].get('caller_identity', {}).get('account_id', 'N/A')}")
