...
class PyramidalOMETiffReader:
            ...
            try:
                ...
                if fallback and level > 0:
                    ...
                    for alt_level in range(level - 1, -1, -1):
                        if alt_level in available_levels:
                            try:
                                zarr_array = zarr.open(tiff.aszarr(series=0, level=alt_level, squeeze=False))
                                _ = zarr_array.shape
    ...
    def get_channel(self, level: int = 0, channel: int = 0, use_memmap: bool = False, 
                   fallback: bool = True, warn_on_fallback: bool = True) -> np.ndarray:
        """
