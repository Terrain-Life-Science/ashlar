...
def segment_dapi_stardist(
    dapi_image: np.ndarray, model_name: str = "2D_versatile_fluo", normalize: bool = True
) -> np.ndarray:
    """
...
def segment_dapi_watershed(
    ...
    labels = segmentation.watershed(
        -distance, markers, mask=binary  # Negative for watershed (finds minima)
    )

    return labels.astype(np.int32)


def extract_centroids_from_labels(
    labels: np.ndarray, intensity_image: Optional[np.ndarray] = None
) -> np.ndarray:
    """
...
def register_centroids_icp(
    ...
    if len(ref_centroids) == 0 or len(target_centroids) == 0:
        ...
        return (
            ...
            100.0,
            {"iterations": 0, "num_matches": 0, "converged": False},
        )
    ...
    for iteration in range(max_iterations):
        ...
        if num_matches < min_matches:
            ...
            return (
                shift,
                100.0,
                {"iterations": iteration + 1, "num_matches": int(num_matches), "converged": False},
            )
    ...
    return (
        shift,
        rmse,
        {"iterations": max_iterations, "num_matches": int(np.sum(valid_final)), "converged": False},
    )


def estimate_shift_from_centroids(
    ...
    if ref_image.shape != target_image.shape:
        raise ValueError(f"Image shapes must match: {ref_image.shape} vs {target_image.shape}")
    ...
    if len(ref_centroids) == 0 or len(target_centroids) == 0:
        ...
        return (
            np.array([0.0, 0.0], dtype=np.float64),
            100.0,
            {"ref_centroids": 0, "target_centroids": 0, "segmentation_method": segmentation_method},
        )

    # Register centroids using ICP
    shift_downsampled, error, icp_metadata = register_centroids_icp(
        ref_centroids, target_centroids, **icp_params
    )
