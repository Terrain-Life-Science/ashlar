...
def _apply_transform_torch(
    image: np.ndarray, source_y: np.ndarray, source_x: np.ndarray, order: int = 1
) -> np.ndarray:
    """
    ...
    transformed_tensor = torch.nn.functional.grid_sample(
        image_tensor, grid, mode=mode, padding_mode="zeros", align_corners=True
    )
...
def apply_transform_to_image(
    image: np.ndarray, transform_matrix: np.ndarray, order: int = 1, use_gpu: bool = False
) -> np.ndarray:
    """
    ...
    if use_gpu and is_gpu_available():
        if TORCH_AVAILABLE:
            ...
            transformed = ndimage.map_coordinates(
                image, [source_y, source_x], order=order, mode="constant", cval=0.0, prefilter=False
            )
        else:
...
def apply_transform_to_channel(
    ...
    if use_tiled:
        ...
        if use_memmap:
            ...
            temp_file.close()
            transformed = np.memmap(temp_file.name, dtype=np.float32, mode="w+", shape=(h, w))
            transformed[:] = 0.0
        ...
        if use_memmap:
            ...
            weight_map = np.memmap(temp_file_weight.name, dtype=np.float32, mode="w+", shape=(h, w))
            weight_map[:] = 0.0
        ...
        try:
            ...
            for tile_info in grid:
                ...
                transformed[y0 : y0 + th, x0 : x0 + tw] += (
                    transformed_tile.astype(np.float32) * tile_weight
                )
        ...
        # Apply transform
        transformed = apply_transform_to_image(
            image, transform_matrix, order=order, use_gpu=use_gpu
        )

        return transformed


def apply_transform_to_all_channels(
    reader: PyramidalOMETiffReader, transform_matrix: np.ndarray, level: int = 0, order: int = 1
) -> list:
    """
    ...
    try:
        if use_memmap:
            ...
            for ch_idx in range(num_channels):
                ...
                transformed_ch = np.memmap(
                    temp_file.name, dtype=np.float32, mode="w+", shape=(h, w)
                )
                transformed_ch[:] = 0.0
                ...
                weight_map = np.memmap(
                    temp_file_weight.name, dtype=np.float32, mode="w+", shape=(h, w)
                )
                weight_map[:] = 0.0
        ...
        for channel_idx in range(num_channels):
            ...
            for tile_info in grid:
                ...
                # Accumulate weighted tile values
                transformed_channels[channel_idx][y0 : y0 + th, x0 : x0 + tw] += (
                    transformed_tile.astype(np.float32) * tile_weight
                )
            ...
            if use_memmap:
                ...
                transformed_uint16 = np.memmap(
                    uint16_file_path, dtype=np.uint16, mode="w+", shape=(h, w)
                )
                ...
                for row_start in range(0, h, chunk_rows):
                    ...
                    transformed_uint16[row_start:row_end, :] = np.clip(chunk, 0, 65535).astype(
                        np.uint16
                    )
