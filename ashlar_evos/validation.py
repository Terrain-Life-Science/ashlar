...
def calculate_image_similarity(
    ...
    if image1.shape != image2.shape:
        raise ValueError(
            f"Images must have same shape: {image1.shape} vs {image2.shape}"
        )
    ...
    if calculate_ssim:
        try:
            ...
            ssim = structural_similarity(
                image1, image2, data_range=data_range
            )
        except ImportError:
...
def validate_registration_accuracy(
    ...
    if calculated_transforms is not None and expected_transforms is not None:
        if len(calculated_transforms) != len(expected_transforms):
            ...
            transform_validation = {
                ...
                'mean_rmse': float(np.mean([e['rmse'] for e in transform_errors])) if transform_errors else 0.0
            }
    ...
    overall_accuracy = {
        ...
        'is_accurate': (
            shift_validation['within_tolerance'] and
            rmse_combined < 5.0  # Reasonable threshold for sub-pixel accuracy
        )
    }
    ...
    if transform_validation:
        ...
        if transform_validation.get('is_valid', False):
            overall_accuracy['is_accurate'] = (
                overall_accuracy['is_accurate'] and
                transform_validation['mean_rmse'] < 1.0
            )
