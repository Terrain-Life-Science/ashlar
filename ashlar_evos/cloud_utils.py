...
def calculate_optimal_pyramid_level(
    image_width: int, image_height: int, cycle_file: Path, target_effective_size: int = 256
) -> int:
    """
...
def suggest_cloud_parameters(
    ...
    optimal_level = calculate_optimal_pyramid_level(image_width, image_height, cycle_file)
    ...
    if check_aws_quota and AWS_AVAILABLE:
        ...
        if not gpu_quota_info["can_launch_gpu"] and result["gpu_recommended"]:
            ...
            result["gpu_recommended_note"] = (
                "GPU recommended but quota unavailable - use CPU instance"
            )
...
def detect_large_image(
    image_width: int, image_height: int, tile_size: int = 4096, tile_overlap: int = 512
) -> Tuple[bool, Dict]:
    """
...
def recommend_aws_instance_type(
    image_width: int, image_height: int, gpu_quota_available: bool = True, prefer_gpu: bool = True
) -> Dict[str, any]:
    """
    ...
    if not result["recommended_instance"] or not result["gpu_available"]:
        if image_dimension >= 30000:
            ...
            result["reasoning"] = f"Smaller image ({image_dimension}px). c5.xlarge is sufficient."
...
def parse_s3_path(s3_path: str) -> Dict[str, str]:
    ...
    return {"bucket": bucket, "key": key, "valid": bucket is not None}


def optimize_for_s3_paths(
    file_paths: list, check_bucket_exists: bool = False, region: str = "us-east-1"
) -> Dict[str, any]:
    """
...
def estimate_aws_instance_cost(
    instance_type: str, region: str = "us-east-1", hours: float = 1.0, use_spot: bool = False
) -> Dict[str, any]:
    """
...
def estimate_registration_cost(
    ...
    if instance_type is None:
        ...
        instance_rec = recommend_aws_instance_type(
            image_width, image_height, gpu_quota_available=gpu_quota_available, prefer_gpu=True
        )
    ...
    result = {
        ...
        "instance_type": instance_type,
        "instance_cost": cost_info.get("estimated_cost_on_demand")
        or cost_info.get("estimated_cost_spot"),
        "data_transfer_cost": data_transfer_cost,
        "total_estimated_cost": (cost_info.get("estimated_cost_on_demand") or 0)
        + data_transfer_cost,
        "recommendations": [],
    }
