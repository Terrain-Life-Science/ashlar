...
def find_sample_directories(base_dir: pathlib.Path, pattern: str = "*") -> List[pathlib.Path]:
    ...
    sample_dirs = [pathlib.Path(d) for d in glob.glob(str(pattern_path)) if pathlib.Path(d).is_dir()]
...
Examples:
  ...
  python -m ashlar_evos.scripts.register_batch --input-dir ./samples --output-dir ./aligned --pattern "sample_*" --num-workers 8
    ...
    parser.add_argument(
        '--cloud',
        action='store_true',
        help='Enable cloud optimizations'
    )
    ...
    if not sample_dirs:
        print(f"Error: No sample directories found matching pattern '{args.pattern}' in {args.input_dir}")
        return 1
    ...
    worker_args = [
        (sample_dir, args.output_dir, pipeline_kwargs)
    ...
    with ProcessPoolExecutor(max_workers=args.num_workers) as executor:
        # Submit all tasks
        future_to_sample = {
            executor.submit(register_single_sample, args): args[0]
