    ...
    try:
        with open(filepath) as f:
            ...
            if isinstance(data, list):
                ...
                print("Warning: Unexpected format in ground truth file")
                return None
...
Examples:
  ...
  validate_registration cycle_00.ome.tif cycle_01.ome.tif --report report.json
    ...
    parser.add_argument("reference", type=pathlib.Path, help="Reference cycle OME-TIFF file")
    ...
    parser.add_argument(
        "--ground-truth-file", type=pathlib.Path, help="Path to JSON file with ground truth shifts"
    )
    ...
    parser.add_argument(
        "--save-metrics", type=pathlib.Path, help="Save validation metrics to JSON file"
    )

    parser.add_argument(
        "--max-shift", type=float, help="Maximum allowed shift magnitude in pixels for validation"
    )
        ...
        try:
            ...
            if registered_img.shape != reference_img.shape:
                ...
                if registered_img.size != reference_img.size:
                    print("  Skipping due to size mismatch")
                    continue
            ...
            all_metrics.append(
                {"file": str(reg_file), "similarity": similarity.to_dict(), "overlap": overlap}
            )
    ...
    if calculated_shifts and expected_shifts:
        ...
        if len(calc_shifts) == len(expected_shifts[1 : len(args.registered) + 1]):
            ...
            all_metrics.append({"shift_validation": validation})
