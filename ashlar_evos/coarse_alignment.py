...
def read_pyramid_dapi(
    filepath: Path, level: int = 3, dapi_channel: int = 0, use_memmap: bool = False
) -> np.ndarray:
    """
...
def coarse_align(
    ...
    if reference_dapi.shape != target_dapi.shape:
        raise ValueError(f"Image shapes must match: {reference_dapi.shape} vs {target_dapi.shape}")

    # Use ashlar's phase correlation with specified upsampling
    try:
        shift, error = utils.register(
            reference_dapi, target_dapi, sigma=filter_sigma, upsample=upsample
        )
...
def coarse_align_cycle(
    ...
    try:
        ref_dapi = read_pyramid_dapi(
            reference_file, level=pyramid_level, dapi_channel=dapi_channel, use_memmap=use_memmap
        )
        target_dapi = read_pyramid_dapi(
            target_file, level=pyramid_level, dapi_channel=dapi_channel, use_memmap=use_memmap
        )
    except Exception as e:
    ...
    shift, error = coarse_align(
        ref_dapi, target_dapi, filter_sigma=filter_sigma, upsample=upsample, max_error=max_error
    )
...
def coarse_align_all_cycles_centroid(
