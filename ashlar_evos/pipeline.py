...
from .cloud_utils import calculate_optimal_pyramid_level, detect_large_image, optimize_worker_count
...
class EvosRegistrationPipeline:
    ...
    def __init__(
        ...
        if image_size is not None and image_size.get("width") and image_size.get("height"):
            ...
            try:
                optimal_level = calculate_optimal_pyramid_level(
                    image_size["width"], image_size["height"], self.cycle_files[0]
                )
                if self.verbose:
                    self._print(
                        f"Auto-configured pyramid level: {optimal_level} "
        ...
        if image_size is not None and image_size.get("width") and image_size.get("height"):
            is_large, recommendations = detect_large_image(
                image_size["width"], image_size["height"], self.tile_size, self.tile_overlap
            )
            if is_large:
                self._print(
                    f"Large image detected: {recommendations['image_area_pixels']:,} pixels"
                )
    ...
    def _validate_input_files(self):
        ...
        try:
            ...
            for i, f in enumerate(self.cycle_files):
                ...
                @retry_with_backoff(
                    max_attempts=3, initial_delay=0.5, exceptions=(IOError, OSError)
                )
                ...
                try:
                    ...
                    if channels != ref_channels:
                        ...
                        cycle_mismatches.append(
                            f"{channels} channels != reference {ref_channels} channels"
                        )

                    if levels != ref_levels:
                        cycle_incompatible = True
                        cycle_mismatches.append(
                            f"{levels} pyramid levels != reference {ref_levels} levels"
                        )
                    ...
                    if cycle_incompatible:
                        ...
                        mismatches.append(f"  Cycle {i} ({f.name}): " + ", ".join(cycle_mismatches))

                    meta.close()
                except Exception as e:
                    incompatible_cycles.append(i)
                    mismatches.append(f"  Cycle {i} ({f.name}): Error reading metadata - {e}")
            ...
            if mismatches:
                ...
                if self.skip_incompatible_cycles:
                    ...
                    self.cycle_files = [
                        f for i, f in enumerate(self.cycle_files) if i not in incompatible_cycles
                    ]
    ...
    def _check_memory_usage(self):
        ...
        if (
            not self.image_size
            or not self.image_size.get("width")
            or not self.image_size.get("height")
        ):
            return
        ...
        try:
            with OMEMetadata(self.cycle_files[0]) as meta:
                ...
                try:
                    ...
                    level_area = (self.image_size["width"] * self.image_size["height"]) / (
                        2 ** (pyramid_level * 2)
                    )
        except Exception:
            num_channels = 3
        ...
        state = CheckpointState(
            ...
            transforms={
                str(k): {
                    "transform": (
                        v["transform"].tolist()
                        if "transform" in v and isinstance(v["transform"], np.ndarray)
                        else v.get("transform")
                    ),
    ...
    def load_checkpoint(self, checkpoint_path: Path) -> "CheckpointState":
        ...
        for k, v in state.fine_shifts.items():
            ...
            if v:
                shifts = [
                    (np.array(s[0]) if isinstance(s[0], list) else s[0], float(s[1])) for s in v
                ]
    ...
    def run_coarse_alignment(self) -> Dict[int, Tuple[np.ndarray, float]]:
        ...
        with self.performance_monitor.phase("Coarse Alignment"):
            ...
            if self.coarse_only:
                ...
                upsample = 10
                self._print_clean(
                    f"  Using pyramid level {pyramid_level} (4x downsampled) with {upsample}x upsampling for sub-pixel accuracy"
                )
            else:
                ...
                if self.image_size and pyramid_level == 3:
                    image_dimension = max(
                        self.image_size.get("width", 0), self.image_size.get("height", 0)
                    )
                    if image_dimension >= 8192:  # 4x, 8x, and 16x images
                        upsample = 10
                        self._print_clean(
                            f"  Using pyramid level {pyramid_level} with {upsample}x upsampling for improved precision on large image ({image_dimension}×{image_dimension})"
                        )
                    else:
                        upsample = 1
            ...
            for i, (shift, error) in self.coarse_shifts.items():
                if i != self.reference_idx:
                    ...
                    self._print_clean(
                        f"  Cycle {i}: shift=({shift[0]:.2f}, {shift[1]:.2f}), error={error:.4f}"
                    )
        ...
        with self.performance_monitor.phase(f"Fine Registration - Cycle {cycle_idx}"):
            ...
            try:
                ...
                if not self._validate_shift(coarse_shift, cycle_idx):
                    self._print_clean(
                        "Proceeding with fine registration despite extreme coarse shift",
                        level="WARNING",
                    )
    ...
    def fit_transform(self, cycle_idx: int) -> Dict:
        ...
        if self.coarse_only:
            ...
            with self.performance_monitor.phase(f"Transform Creation - Cycle {cycle_idx}"):
                ...
                self.transforms[cycle_idx] = result
                self._print_clean(
                    f"  Translation: ({coarse_shift[1]:.4f}, {coarse_shift[0]:.4f}) pixels"
                )
    ...
    def apply_transform(self, cycle_idx: int, output_file: Path) -> None:
        ...
        with self.performance_monitor.phase(f"Apply Transform - Cycle {cycle_idx}"):
            ...
            if use_gpu:
                self._print_clean("Using GPU acceleration for transform")
    ...
    def run_full_pipeline(
            ...
            for i in range(len(self.cycle_files)):
                if i != self.reference_idx:
                    if i in self.completed_cycles:
                        self._print_clean(
                            f"Phase 2: Fine Registration - Cycle {i} - SKIPPED (already completed)"
                        )
                        print()
                    else:
        ...
        for i in range(len(self.cycle_files)):
            if i in self.transforms and i in self.completed_cycles:
                self._print_clean(
                    f"Phase 3: Transform Fitting - Cycle {i} - SKIPPED (already completed)"
                )
                print()
            else:
        ...
        for i in range(len(self.cycle_files)):
            ...
            if output_file.exists() and i in self.completed_cycles:
                self._print_clean(
                    f"Phase 4: Apply Transform - Cycle {i} - SKIPPED (output file exists)"
                )
                print()
        ...
        if report_path:
            self.performance_monitor.generate_report(
                str(report_path), scale_factor=self.scale_factor, image_size=self.image_size
            )
            self._print_status(f"Performance report saved to: {report_path}")

        return output_files

    def run_alignment_only(self, output_path: Path, report_path: Optional[Path] = None) -> Dict:
        """
        ...
        self._print_clean(
            f"Total time: {self.performance_monitor.metrics.elapsed_time():.2f} seconds"
        )
        ...
        for cycle in result["cycles"]:
            if cycle["index"] == self.reference_idx:
                ...
                self._print_clean(
                    f"  Cycle {cycle['index']}: shift=({shift[0]:.2f}, {shift[1]:.2f}), error={error:.4f}"
                )
        print()
