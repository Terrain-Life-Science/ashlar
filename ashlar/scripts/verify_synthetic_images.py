        ...
        with tifffile.TiffFile(filepath) as tiff:
            ...
            if len(tiff.series) > 0:
                ...
                if len(tiff.pages) > 0:
                    page = tiff.pages[0]
                    results["compression"] = (
                        str(page.compression) if hasattr(page, "compression") else "unknown"
                    )
...
def compare_cycles(cycle_files: List[pathlib.Path]):
    ...
    for filepath, results in all_results:
        ...
        if results["num_channels"] != ref_results["num_channels"]:
            match = False
            issues.append(
                f"Channel count mismatch: {results['num_channels']} vs {ref_results['num_channels']}"
            )

        if results["num_series"] != ref_results["num_series"]:
            match = False
            issues.append(
                f"Pyramid levels mismatch: {results['num_series']} vs {ref_results['num_series']}"
            )
...
Examples:
  ...
  python -m ashlar.scripts.verify_synthetic_images --visualize --level 2
    ...
    parser.add_argument(
        "--visualize", "-v", action="store_true", help="Display images using matplotlib"
    )

    parser.add_argument(
        "--level", type=int, default=0, help="Pyramid level to visualize (default: 0 = base level)"
    )

    parser.add_argument(
        "--channel", type=int, default=0, help="Channel to visualize (default: 0 = DAPI)"
    )
    ...
    if len(cycle_files) == 0:
        ...
        print("   Expected files: cycle_00.ome.tif, cycle_01.ome.tif, cycle_02.ome.tif")
        return 1
